...
const CataloguePage = () => {
  ...
  // Get unique categories from traits or create based on traits
  const getUniqueCategories = () => {
    const categories = new Set<string>();
    categories.add("all");
    catalogueItems.forEach((item) => {
      if (item.traits && item.traits.length > 0) {
        // Use first trait as category or create categories
        const category = item.traits[0];
        categories.add(category);
      }
    });
    // Add some default categories if none exist
    if (categories.size === 1) {
      ["Spiritual", "Healing", "Guidance", "Reading"].forEach((cat) =>
        categories.add(cat),
      );
    }
  ...
  // Filter and search logic
  const filteredData = catalogueItems
    .filter(
      (item) =>
        item.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
        (item.traits &&
          item.traits.some((trait) =>
    ...
    .filter(
      (item) =>
        filters.category === "all" ||
        (item.traits && item.traits.includes(filters.category)),
    )
    .filter((item) => item.rating >= filters.minRating)
    .sort((a, b) => {
      switch (filters.sortBy) {
  ...
  const handleBookNow = (itemId: string | number) => {
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            <p
              ...
              style={{ fontFamily: "var(--font-montserrat)" }}
            >
              Showing {filteredData.length} of {catalogueItems.length} readings
            </p>
          </motion.div>
              ...
              {paginatedData.map((item, index) => (
                ...
                >
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    ...
                    {/* Book Now button pinned to bottom */}
                    <button
                      onClick={() => handleBookNow(item.id || item._id!)}
                      className="mt-auto w-full bg-black text-white font-medium py-3 px-6 rounded-xl hover:bg-gray-800 transition-all cursor-pointer text-center"
                      style={{ fontFamily: "var(--font-montserrat)" }}
                    >
                      Book Now
